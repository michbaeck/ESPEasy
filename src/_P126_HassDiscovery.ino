...
/* by michael baeck
  ...
  * stay tuned for more plugins like homeassistant-switch, etc
  ...
  bugs/todo
    [todo] no command return (commands working though)
...
// force using companion plugin when using this one
#ifndef USES_P128
 #define USES_P128
#endif
...
// struct + global vars
  ...
  #define _P126_DEBUG
  // #define _P126_S(start)  (_P126_log = F("[P126] ")\; _P126_log = F(start);)
  // #define _P126_P(push)  (_P126_log += toString(push); addLog(LOG_LEVEL_DEBUG, _P126_log);)
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
    {
      ...
      // command buttons
        ...
        if (Settings.TaskDeviceEnabled[event->TaskIndex] && !_P126_cleaning) {
          ...
          int duration = TASKS_MAX * 3 * _P126_INTERVAL /1000/60;
          int payloads = TASKS_MAX * 3 * 4;
        ...
        if (_P126_cleaning) {
          html_BR();
          int duration = (TASKS_MAX * 3 - (_P126_increment + 1)) * _P126_INTERVAL / 1000;
    ...
    case PLUGIN_WRITE:
    {
      ...
      if (tmpString.equalsIgnoreCase(F("discovery"))) {
        ...
        _P126_log = F("[P126] command issued : ");
        _P126_log += string;
    ...
    case PLUGIN_ONCE_A_SECOND:
    {
      if (_P126_cleaning && timeOutReached(_P126_time + _P126_INTERVAL)) {
        ...
        if (_P126_increment >= TASKS_MAX * 3 - 1) {
...
int _P126_find_sensors(struct DiscoveryStruct *discovery, bool save) {
  _P126_log = F("[P126] search active tasks: state of save is : ");
      ...
      if (Settings.TaskDeviceEnabled[TaskIndex]) {                                            // if task enabled
        ...
        if (ctrlenabled) {
          if (Device[DeviceIndex].ValueCount != 0) {                                          // only tasks with values
            _P126_log = F("[P126] found active task with id : ");
            ...
            for (byte x = 0; x < VARS_PER_TASK; x++) {                                        // for each value
              if (x < Device[DeviceIndex].ValueCount && !(x > 0 && pluginid == _P126_HASWITCH)) {
                if (String(ExtraTaskSettings.TaskDeviceValueNames[x]).length() > 0) {
                  _P126_log = F("[P126] found value with name : ");
...
bool _P126_system_config(struct DiscoveryStruct *discovery, bool brief) {
  ...
  //  create discovery payload
    ...
    if (Settings.TaskDeviceTimer[discovery->taskid] == 0) {
      ...
      #endif
        _P126_log = F("[P126] payload created : ");
        _P126_log += payload;
        addLog(LOG_LEVEL_DEBUG,_P126_log);
    }
  ...
  //  check payload size and publish                     
    if (_P126_check_length(discovery_topic, payload)) {
      ...
      addLog(LOG_LEVEL_ERROR, F("[P126] Payload exceeds limits. Trying with reduced content."));
      return (_P126_system_config(discovery, true)); // start over with brief-flag
    } else {
      _P126_log = F("[P126] payload exceeds limits. You can publish the message manually from other client. topic : ");
      _P126_log += discovery_topic;
      addLog(LOG_LEVEL_ERROR,_P126_log);
      _P126_log = F("[P126] payload : ");
      _P126_log += payload;
      addLog(LOG_LEVEL_ERROR,_P126_log);
        Serial.println(_P126_log);
      return false;
        ...
        if (discovery->save.task[TaskIndex].value[x].enable && !(x > 0 && pluginid == _P126_HASWITCH)) {    // if value enabled
          ...
          //  check and publish payload
            if (_P126_check_length(discovery_topic, payload)) {
              ...
              addLog(LOG_LEVEL_ERROR, F("[P126] Payload exceeds limits. Trying again with reduced content."));
              success = _P126_sensor_config(discovery, true);
            } else {
              _P126_log = F("[P126] Cannot publish, payload exceeds limits. You can publish the message manually : ");
              _P126_log += payload;
              addLog(LOG_LEVEL_ERROR,_P126_log);
...
String _P126_make_topic(char* prefix, String component, String unique) {
    ...
    topic += unique;
    topic += F("/config");
      _P126_log = F("[P126] topic defined : ");
      _P126_log += topic;
      addLog(LOG_LEVEL_DEBUG,_P126_log);
...
bool _P126_system_state(struct DiscoveryStruct *discovery, bool brief) {
  ...
  // workaround for issues with not updating LWT to "online" 
  // for nodes with frequent wifi disconnects due to bad signal
    _P126_publish(discovery->ctrlid, discovery->lwttopic, discovery->lwtup);
...
bool _P126_cleanup(struct DiscoveryStruct *discovery) {
  ...
  byte lasttaskid = TASKS_MAX - 1;    // 0-11 or 0-31
  byte taskid = _P126_increment;    // 0
  String component = F("sensor");

  if (_P126_increment > lasttaskid + TASKS_MAX) {   // 24-35 or 64-95
    taskid = _P126_increment - TASKS_MAX*2;   // 0
    component = F("switch");
  } 

  if (_P126_increment > lasttaskid) { // 12-23 or 32-63
    taskid = _P126_increment - TASKS_MAX;   // 0
    component = F("binary_sensor");
  } 
...
bool _P126_check_length(const String& topic, const String& payload) {
  ...
  _P126_log = F("[P126] payload size is : ");
...
bool _P126_publish(int ctrlid, const String& topic, const String& payload) {
  bool success = false;
  _P126_log = F("[P126] publish : topic : ");
  _P126_log += topic;
  addLog(LOG_LEVEL_DEBUG,_P126_log);
    Serial.println(_P126_log);
  
  if (MQTTCheck(ctrlid)) {
    _P126_log = F("[P126] publish : controller check ok");
    //_P126_log += payload;
    addLog(LOG_LEVEL_DEBUG,_P126_log);
      Serial.println(_P126_log);
    
    if (ctrlid >= 0) {
      _P126_log = F("[P126] publish : payload : ");
      _P126_log += payload;
      addLog(LOG_LEVEL_DEBUG,_P126_log);
...
void _P126_get_ctrl(struct DiscoveryStruct *discovery) {
  ...
  _P126_log = F("[P126] get_ctrl : loading controller settings for id : ");
  _P126_log += String(discovery->ctrlid);
  addLog(LOG_LEVEL_DEBUG,_P126_log);
  ...
  _P126_log = F("[P126] get_ctrl : loaded cmdtopic : ");
  _P126_log += discovery->cmdtopic;
  addLog(LOG_LEVEL_DEBUG,_P126_log);

  discovery->publish = ControllerSettings.Publish;
  discovery->publish.replace(F("%sysname%"), Settings.Name);

  // _P126_S("get_ctrl : load pubtopic : ");
  // _P126_P(discovery->publish);
...
bool _P126_get_id(struct DiscoveryStruct *discovery) {
  ...
  for (byte x = 0; x < TASKS_MAX; x++) {
    ...
    if (pluginid == PLUGIN_ID_126) {
      discovery->taskid = x;
      _P126_log = F("[P126] found correct taskid: ");
      _P126_log += String(x);
      addLog(LOG_LEVEL_DEBUG,_P126_log);
      success = true;
      break;
  ...
  if (!success) {
    _P126_log = F("[P126] couldn't fetch taskid");
    addLog(LOG_LEVEL_ERROR,_P126_log);
  }
